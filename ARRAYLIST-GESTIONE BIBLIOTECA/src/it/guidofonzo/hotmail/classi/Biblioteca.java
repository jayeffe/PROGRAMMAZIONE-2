...
public void BubbleSort() {
	...
	int i=0;
	while(i<utenti.size()&& !ordinato) {
		ordinato=true;
		for(int j=0; j<utenti.size()-i-1;j++) {
			if(utenti.get(j).compareTo(utenti.get(i+1))>0){
				Utente dipTemp=utenti.get(j);
				utenti.set(j, utenti.get(j+i));
				utenti.set(j+1, dipTemp);
				ordinato=false;
			}
		}
		i++;
